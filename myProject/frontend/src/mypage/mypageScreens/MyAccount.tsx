...
export default function MyAccount() {
  ...
  const idCheck = () => {
    if (memberId.trim() === '') {
      ...
      axios
        .post(config.address + 'idCheck', null, {params: {memberId: memberId}})
  ...
  const regist = () => {
    if (memberId.trim() === '') {
      ...
      if (chkPassword && chkId && checkEmail) {
        ...
        return Alert.alert(
          '8자리 이상, 영문(소문자/대문자), 숫자, 특수문자 모두 포함해야 합니다.',
        );
      else if (!chkId)
        return Alert.alert(
          '아이디는 6자리 포함되어야 하고 영문자가 포함되어야 합니다.',
        );
