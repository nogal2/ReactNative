...
export default function UploadScreen() {
  ...
  const RecipeUploadBtn = () => {
    ...
    const uploadRecipe = () => {
      axios
        .get(config.address + 'insertRecipe', {
          params: {
    ...
    const uploadRecipeThumbnailImg = (temp: any) => {
      ...
      axios
        .post(config.address + 'uploadRecipeImg', null, {
          params: {
            docsSeq: temp,
    ...
    const uploadRecipeContentImg = (temp: any) => {
      ...
      for (let i = 0; i < tests.length; i++) {
        ...
        axios
          .get(config.address + 'uploadRecipeImg', {
            params: {
              docsSeq: temp,
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.titleframe}>
          <TextInput
            ...
            value={recipeTitle}
            onChangeText={recipeTitle => setTitle(recipeTitle)}></TextInput>
        </View>
        ...
        <View style={styles.picture}>
          <TouchableOpacity onPress={titleimgshow} style={styles.camera1}>
            <Icon
              ...
              size={100}></Icon>
            <TextInput style={styles.cameraText1}>
              요리 대표 사진을 등록해 주세요.
            </TextInput>
            <TextInput>
              음식사진 외 사람/동물 등의 사진은 삼가해 주세요.
            </TextInput>
            <Image source={{uri: titleimgurl}} style={styles.camera2}></Image>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.contentframe}>
          <TextInput
            style={styles.textinput}
            value={recipeContent}
            onChangeText={recipeContent => setContent(recipeContent)}
            placeholder="&#10;이 레시피를 소개하는 내용을 적어주세요.&#10;예) 남편의 생일을 맞아 소고기 미역국을 끓여봤어요.&#10;어머니로부터 배운 미역국 레시피를 남편의 입맛에 맞게 고안했습니다."
            multiline={true}></TextInput>
        </View>
        <View>{list.map((item: any, i: any) => item)}</View>
        ...
        <View style={styles.categoryframe}>
          ...
          <View style={styles.picker}>
            <RNPickerSelect
              onValueChange={value => setPickerSelect2(value)}
              items={values2}
              placeholder={{
                label: '소분류',
              }}></RNPickerSelect>
        ...
        <View style={styles.categoryframe}>
          <View style={styles.picker}>
            <RNPickerSelect
              onValueChange={value => setPickerSelect3(value)}
              items={values3}
              placeholder={{
                label: '선택',
              }}></RNPickerSelect>
        ...
        <View style={styles.youtubeFrame}>
          <Text style={styles.tagText2}>
            작성하신 레시피의 조리 영상이 있다면 유튜브 주소를 남겨주세요.
          </Text>
        </View>
        <View style={styles.priceframe}>
          <TextInput
            style={styles.textinput}
            value={youtubeurl}
            onChangeText={youtubeurl => setYoutubeurl(youtubeurl)}
            placeholder="https://www.youtube.com/"></TextInput>
        </View>
        ...
        <View style={styles.tagFrame}>
          <Text style={styles.tagText2}>
            재료, 목적, 효능, 대상 등을 입력 후 완료키를 눌러서 태그로
            남겨주세요.{'\n'}예) 돼지고기, 다이어트, 비만, 칼슘, 감기예방,
            이유식, 초간단
          </Text>
        </View>
        <View style={styles.goodsframe}>
          <TagInput
            updateState={(tags: any) => {
              setTags(tags);
            }}
            tags={tags}
            placeholder="Tags..."
        ...
        <View style={styles.priceframe}>
          <TextInput
            style={styles.textinput}
            value={recipePrice}
            onChangeText={recipePrice => setPrice(recipePrice)}
            keyboardType="number-pad"
            placeholder="예) 1000, 5000 숫자를 입력 원단위"></TextInput>
        </View>
        <Button style={styles.btn} onPress={RecipeUploadBtn}>
          레시피작성
        </Button>
      </ScrollView>
    </View>
