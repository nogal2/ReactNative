...
const DetailList = (props: any) => {
  ...
  useEffect(() => {
    const test = () => {
      ...
      console.log(
        '넘어왔니?' +
          JSON.stringify(props.setData3) +
          ' ' +
          parseInt(props.setData4),
      );
      console.log(props.setData3[parseInt(props.setData4)]);
      console.log(
        config.photo + props.setData3[parseInt(props.setData4)]['imglist'],
      );
  ...
  return (
    <View style={styles.contentpictureframe}>
      <TouchableOpacity
        onPress={contentimgshow1}
        style={styles.contentcameraframe}>
        <Image
          style={styles.contentcamera}
          source={{uri: contentImglist == '' ? '/' : contentImglist}}></Image>
      </TouchableOpacity>

      <View style={styles.contentframe}>
        <TextInput
          style={styles.photoContentText}
          value={contentText}
          onChangeText={contentText => setContentText(contentText)}
          onKeyPress={content}
          multiline={true}
          placeholder="레시피 순서를 차례대로 입력해주세요.&#10;+ 버튼을누르시면 순서창이 추가됩니다."
          underlineColor="white"></TextInput>
      </View>
    </View>
  );
};
