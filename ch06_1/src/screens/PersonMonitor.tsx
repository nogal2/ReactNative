...
const PersonBasic:FC<PersonProps> = ({person, deletePressed}) => {
    ...
    useEffect(() => {
        const id = animValue.addListener((state: {value: number}) => { // 현재 보간중인 값을 얻을 수 있음.
    ...
    const avatarPressed = useCallback (
        () =>
            Animated.timing(animValue, {
                ...
                toValue:1,
                duration: 3000,
                easing: Easing.bounce}).start(() => setAnimationEnd((notUsed) => true)), []
    )
    ...
    return (
        <ThemeView>
            ...
            <ThemeText style={[{fontSize:20}]}>animationEnd: {animationEnd ? 'true' : 'false'}</ThemeText>
